...
async function processTestCase(timeAllotment) {
    ...
    await db.sremAsync(constants.inQueueKey, filedata.secret);

    try {
        if (filedata.type == "zip") {
            await cw
                .process(
                    `wget -q -P downloads ${constants.webServerIP}/${folderkey}`
                )
                .death();
        ...
        try {
            ...
            if (timedOut) {
                ...
                if (totalTimeout < constants.timeouts.maxTotalExecution) {
                    result.runtimeError +=
                        `\nYour submission was given a ${
                            totalTimeout / 1000
                        }s to finish all test cases, based on the size of the current queue.\n` +
...
async function complete(filedata, results) {
    ...
    if (results.success) {
        ...
        let name = await db.hgetAsync(
            constants.secretsNamesKey,
            filedata.secret
        );
        console.log(name);
